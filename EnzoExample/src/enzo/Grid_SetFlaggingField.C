...
int grid::SetFlaggingField(int &NumberOfFlaggedCells, int level)
{
  ...
  for (method = 0; method < MAX_FLAGGING_METHODS; method++) {
    ...
    /* ==== METHOD 817: Refine on the center of the box (regardless of the physical quantities) ==== */
    
      case 817:
    if (level < MustRefineRegionMinRefinementLevel) {
      // printf("817\n");
      NumberOfFlaggedCells = this->FlagCellsToBeRefinedByZoomedMustRefineRegion(level);
      if (NumberOfFlaggedCells < 0) {
        ENZO_FAIL("Error in grid->FlagCellsToBeRefinedByMustRefineRegion.");
      }
    }
