...
int grid::SolveRateAndCoolEquations(int RTCoupledSolverIntermediateStep)
{
  ...
  int MetalNum = 0, SNColourNum = 0, MetalIaNum = 0;
  int MetalFieldPresent = FALSE;

  // First see if there's a metal field (so we can conserve species in
  // the solver)
  // Modified 2018.03.06 Change the field of SNColour to MetalIa when SuperTypeIa exists.
  // Replaced the following conditions regarding SNColourNum to MetalIaNum.
  ...
  // printf("MetalNum:%d, SNColourNum:%d, MetalIaNum:%d\n", MetalNum, SNColourNum, MetalIaNum);
  MetalFieldPresent = (MetalNum != -1 || MetalIaNum != -1);
  ...
  if (MetalNum != -1 && SNColourNum != -1 ) {
    ...
    for (i = 0; i < size; i++)
      //fprintf(stdout,"SNColourNum in SolveRateAndCoolEquations");
      TotalMetals[i] = BaryonField[MetalNum][i] + BaryonField[SNColourNum][i]; 
      // Changed back to SNColourNum... since Iam using Pop III SNColour now.
      // Big error discovered so late. QQ 
      // 2019.04.20 lhchen
