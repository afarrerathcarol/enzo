...
{
  // printf("!!!!!!!!!!CollapseTestInitialize Called!!!!!!!!!!!\n");
  ...
  /* Added 2018.03.04 */
  const char *MetalIaName = "MetalIa_Density";
  /* Added 2018.03.04 */
  ...
  CollapseTestSphereSubType[MAX_SPHERES],
  ...
  float CollapseTestSphereSubTypeSlope[MAX_SPHERES];
  ...
  /* 2018.03.06 added*/
  int n_bubble = 10, metal_case_num = 2;
  ...
  for (sphere = 0; sphere < MAX_SPHERES; sphere++) {
    ...
    for (int ii=0; ii < MAX_BUBBLES; ii++) {
      ...
      BubbleMetallicity[sphere][ii] = 1.e-20;
      BubbleTemperature[sphere][ii] = 100000000;
    ...
    for (dim = 0; dim < MAX_DIMENSION; dim++) {
      ...
      for (int ii =0; ii < MAX_BUBBLES; ii++) {
        center_of_bubble[sphere][ii][dim] = 0.5*(DomainLeftEdge[dim] +
         DomainRightEdge[dim]);
        BubbleVelocity[sphere][ii][dim] = 0.0;
      }
    }
    CollapseTestSphereType[sphere]       = 0;
    CollapseTestSphereSubType[sphere]    = 0;
    CollapseTestSphereSubTypeSlope[sphere] = 0.;
  ...
  if (sscanf(line, "CollapseTestSphereSubType[%"ISYM"]", &sphere) > 0)
  ret += sscanf(line, "CollapseTestSphereSubType[%"ISYM"] = %"ISYM, &sphere,
  &CollapseTestSphereSubType[sphere]);
  if (sscanf(line, "CollapseTestSphereSubTypeSlope[%"ISYM"]", &sphere) > 0)
  ret += sscanf(line, "CollapseTestSphereSubTypeSlope[%"ISYM"] = %"FSYM, &sphere,
  &CollapseTestSphereSubTypeSlope[sphere]);
  ...
  /* 2018.02.01 added */
  if (sscanf(line, "CollapseTestSphereHMFraction[%"ISYM"]", &sphere) > 0)
  ret += sscanf(line, "CollapseTestSphereHMFraction[%"ISYM"] = %"FSYM, &sphere,
    &CollapseTestSphereHMFraction[sphere]);
  if (sscanf(line, "CollapseTestSphereH2IIFraction[%"ISYM"]", &sphere) > 0)
  ret += sscanf(line, "CollapseTestSphereH2IIFraction[%"ISYM"] = %"FSYM, &sphere,
    &CollapseTestSphereH2IIFraction[sphere]);
  /* 2018.02.01 added */

  /* 2018.03.18 added */
  if (sscanf(line, "CollapseTestSphereNumberOfBubbles[%"ISYM"]", &sphere) > 0)
  ret += sscanf(line, "CollapseTestSphereNumberOfBubbles[%"ISYM"] = %"ISYM, &sphere,
    &NumberOfBubbles[sphere]);
  if (sscanf(line, "CollapseTestSphereMetallicityDistributionCase[%"ISYM"]", &sphere) > 0)
  ret += sscanf(line, "CollapseTestSphereMetallicityDistributionCase[%"ISYM"] = %"ISYM, &sphere,
    &MetallicityDistributionCase[sphere]);  
  /* 2018.03.18 added */

  if (sscanf(line, "CollapseTestSphereVelocityTowardCenter[%"ISYM"]", &sphere) > 0)
  ret += sscanf(line, "CollapseTestSphereVelocityTowardCenter[%"ISYM"] = %"ISYM, &sphere,
    &CollapseTestSphereVelocityTowardCenter[sphere]);
  
  ret += sscanf(line, "CollapseTestSphereDarkMatterHaloMass = %"ISYM,
  ...
  /********************************************************************************************************************/
  /* 2018.04.20 */
  /* read in bubble information */

  FILE *bubble_info;
  char bubble_filename_[MAX_LINE_LENGTH];

  for (sphere = 0; sphere < CollapseTestNumberOfSpheres; sphere++) {
    for (int mkk = 1; mkk < 10; mkk ++) {
      if (MetallicityDistributionCase[sphere] == mkk) {
        sprintf(bubble_filename_, "CTNC_BubbleInitialize_sphere%d_metalic%d.txt", sphere, mkk);
        printf("%s\n", bubble_filename_);
      }    
    }
  ...
  if (MetallicityDistributionCase[sphere] == 1 || MetallicityDistributionCase[sphere] == 4 || MetallicityDistributionCase[sphere] == 5) {
    bubble_info = fopen(bubble_filename_, "r");
    if (bubble_info) printf("Successfully read in CTNC_BubbleInitialize 1 4 5.\n");
    else printf("Fail read in CTNC_BubbleInitialize 1 4 5.\n");
    
    char  bubble_line[MAX_LINE_LENGTH];
    /* read input from CTNC_BubbleInitialize.txt */

    while (fgets(bubble_line, MAX_LINE_LENGTH, bubble_info) != NULL) {

    ret = 0;

    /* read bubble parameters */

    if (sscanf(bubble_line, "CollapseTestSphereBubbleCenter[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0) {
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleCenter[%"ISYM"][%"ISYM"] = %"FSYM" %"FSYM" %"FSYM, 
    &sphere, &bubble, &center_of_bubble[sphere][bubble][0],
    &center_of_bubble[sphere][bubble][1],
    &center_of_bubble[sphere][bubble][2]);
    }
 
    if (sscanf(bubble_line, "CollapseTestSphereBubbleInnerDensity[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleInnerDensity[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleInnerDensity[sphere][bubble]);    
    if (sscanf(bubble_line, "CollapseTestSphereBubbleOuterDensity[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleOuterDensity[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleOuterDensity[sphere][bubble]); 

    if (sscanf(bubble_line, "CollapseTestSphereBubbleInnerMetallicity[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleInnerMetallicity[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleInnerMetallicity[sphere][bubble]); 
    if (sscanf(bubble_line, "CollapseTestSphereBubbleOuterMetallicity[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleOuterMetallicity[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleOuterMetallicity[sphere][bubble]); 

    if (sscanf(bubble_line, "CollapseTestSphereBubbleInnerTemperature[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleInnerTemperature[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleInnerTemperature[sphere][bubble]); 
    if (sscanf(bubble_line, "CollapseTestSphereBubbleOuterTemperature[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleOuterTemperature[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleOuterTemperature[sphere][bubble]); 

    if (sscanf(bubble_line, "CollapseTestSphereBubbleRadius[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleRadius[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleRadius[sphere][bubble]); 
    if (sscanf(bubble_line, "CollapseTestSphereBubbleInnerRadius[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleInnerRadius[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleInnerRadius[sphere][bubble]); 

    /* if the bubble_line is suspicious, issue a warning */

    if (ret == 0 && strstr(bubble_line, "=") && strstr(bubble_line, "CollapseTest") && bubble_line[0] != '#')
    fprintf(stderr, "warning: the following parameter line was not interpreted:\n%s\n", bubble_line);
  ...
  if (MetallicityDistributionCase[sphere] >= 6) {
    bubble_info = fopen(bubble_filename_, "r");
    if (bubble_info) printf("Successfully read in CTNC_BubbleInitialize >=6.\n");
    else printf("Fail read in CTNC_BubbleInitialize >=6.\n");
    ...
    } // MetallicityDistributionCase[sphere] >= 6
    fclose(bubble_info);
  }


  if (MetallicityDistributionCase[sphere] == 2 || MetallicityDistributionCase[sphere] == 3) {
    bubble_info = fopen(bubble_filename_, "r");
    if (bubble_info) printf("Successfully read in CTNC_BubbleInitialize 2 3.\n");
    else printf("Fail read in CTNC_BubbleInitialize 2 3.\n");
    ...
    if (sscanf(bubble_line, "CollapseTestSphereBubbleVelocity[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleVelocity[%"ISYM"][%"ISYM"] = %"FSYM" %"FSYM" %"FSYM, 
    &sphere, &bubble, &BubbleVelocity[sphere][bubble][0],
    &BubbleVelocity[sphere][bubble][1],
    &BubbleVelocity[sphere][bubble][2]);    

    if (sscanf(bubble_line, "CollapseTestSphereBubbleDensity[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleDensity[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleDensity[sphere][bubble]);    

    if (sscanf(bubble_line, "CollapseTestSphereBubbleMetallicity[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleMetallicity[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleMetallicity[sphere][bubble]); 

    if (sscanf(bubble_line, "CollapseTestSphereBubbleTemperature[%"ISYM"][%"ISYM"]", &sphere, &bubble) > 0)
    ret += sscanf(bubble_line, "CollapseTestSphereBubbleTemperature[%"ISYM"][%"ISYM"] = %"FSYM, &sphere, &bubble,
    &BubbleTemperature[sphere][bubble]); 
    ...
    } // MetallicityDistributionCase[sphere] == 2 or 3
    fclose(bubble_info);
  }
  ...
  /*
  for (sphere = 0; sphere < CollapseTestNumberOfSpheres; sphere++) {
  if (NumberOfBubbles[sphere] > 0) {
  for (bubble = 0; bubble < NumberOfBubbles[sphere]; bubble++) {
    printf("Bubble[%d][%d]Center x: %f,  y: %f,  z: %f\n", sphere, bubble, 
    center_of_bubble[sphere][bubble][0],
    center_of_bubble[sphere][bubble][1],
    center_of_bubble[sphere][bubble][2]);
  printf("Bubble[%d][%d]Metallicity: %f\n", sphere, bubble, BubbleMetallicity[sphere][bubble]);
  }      
  }
  }
  */
  ...
  if (TopGrid.GridData->CollapseTestInitializeGrid(
   ...
   CollapseTestSphereDispersion, CollapseTestSphereCutOff, 
   CollapseTestSphereAng1, CollapseTestSphereAng2, 
   CollapseTestSphereNumShells, CollapseTestSphereType,
   CollapseTestSphereSubType, CollapseTestSphereSubTypeSlope, 
   CollapseTestSphereConstantPressure, CollapseTestSphereSmoothSurface, 
   CollapseTestSphereSmoothRadius, CollapseTestSphereHIIFraction, 
   CollapseTestSphereHeIIFraction, CollapseTestSphereHeIIIFraction, 
   CollapseTestSphereH2IFraction, CollapseTestUseParticles, 
   CollapseTestParticleMeanDensity, CollapseTestUniformVelocity, 
   CollapseTestUseColour, CollapseTestUseMetals,
   CollapseTestInitialTemperature, CollapseTestInitialDensity,
   0, CollapseTestInitialFractionHII, 
   CollapseTestInitialFractionHeII, CollapseTestInitialFractionHeIII, 
   CollapseTestInitialFractionHM, CollapseTestInitialFractionH2I, 
   CollapseTestInitialFractionH2II, 
#ifdef TRANSFER
    RadHydroInitialRadiationEnergy,
#endif
   center_of_bubble, 
   MetallicityDistributionCase, NumberOfBubbles,
   BubbleVelocity, BubbleDensity,
   BubbleMetallicity, BubbleTemperature,
   BubbleRadius, BubbleInnerRadius,
   BubbleInnerDensity, BubbleOuterDensity,
   BubbleInnerMetallicity, BubbleOuterMetallicity,
   BubbleInnerTemperature, BubbleOuterTemperature,
   CollapseTestSphereVelocityTowardCenter) == FAIL) {
  ...
  CollapseTestUseColour, CollapseTestUseMetals,
  CollapseTestInitialTemperature, CollapseTestInitialDensity,
  lev-1, CollapseTestInitialFractionHII, 
  CollapseTestInitialFractionHeII, CollapseTestInitialFractionHeIII, 
  CollapseTestInitialFractionHM, CollapseTestInitialFractionH2I, 
  CollapseTestInitialFractionH2II, 
#ifdef TRANSFER
  ...
  // for (int qrs = 0; qrs < MAX_DEPTH_OF_HIERARCHY; qrs++) {
  //   printf("LevelArray[%d]: %d\n", qrs, LevelArray[qrs]);
  // }
  ...
  // printf("levle and RebuildHierarchy\n");
  ...
  if (Temp->GridData->CollapseTestInitializeGrid(
    ...
    CollapseTestUseColour, CollapseTestUseMetals,
    CollapseTestInitialTemperature, CollapseTestInitialDensity,
    level+1, CollapseTestInitialFractionHII, 
    CollapseTestInitialFractionHeII, CollapseTestInitialFractionHeIII, 
    CollapseTestInitialFractionHM, CollapseTestInitialFractionH2I, 
    CollapseTestInitialFractionH2II, 
#ifdef TRANSFER
  ...
  CollapseTestUniformVelocity[2]);
    for (sphere = 0; sphere < CollapseTestNumberOfSpheres; sphere++) {
      ...
      CollapseTestSphereType[sphere]);
      fprintf(Outfptr, "CollapseTestSphereSubType[%"ISYM"] = %"ISYM"\n", sphere,
      CollapseTestSphereSubType[sphere]);
      fprintf(Outfptr, "CollapseTestSphereSubTypeSlope[%"ISYM"] = %"FSYM"\n", sphere,
      CollapseTestSphereSubTypeSlope[sphere]); 
      ...
      /* 2018.03.18 added */
      fprintf(Outfptr, "CollapseTestSphereNumberOfBubbles[%"ISYM"] = %"GOUTSYM"\n", sphere,
      NumberOfBubbles[sphere]);
      fprintf(Outfptr, "CollapseTestSphereMetallicityDistributionCase[%"ISYM"] = %"GOUTSYM"\n", sphere,
      MetallicityDistributionCase[sphere]);
      /* 2018.03.18 added */

      fprintf(Outfptr, "CollapseTestSphereBubbleDensity[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleDensity[sphere][bubble]);

      /* 2018.04.24 */
      fprintf(Outfptr, "CollapseTestSphereBubbleRadius[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleRadius[sphere][bubble]);
      fprintf(Outfptr, "CollapseTestSphereBubbleInnerRadius[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleInnerRadius[sphere][bubble]);  

      fprintf(Outfptr, "CollapseTestSphereBubbleInnerDensity[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleInnerDensity[sphere][bubble]);
      fprintf(Outfptr, "CollapseTestSphereBubbleOuterDensity[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleOuterDensity[sphere][bubble]);      

      fprintf(Outfptr, "CollapseTestSphereBubbleInnerMetallicity[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleInnerMetallicity[sphere][bubble]);
      fprintf(Outfptr, "CollapseTestSphereBubbleOuterMetallicity[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleOuterMetallicity[sphere][bubble]);  

      fprintf(Outfptr, "CollapseTestSphereBubbleInnerTemperature[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleInnerTemperature[sphere][bubble]);
      fprintf(Outfptr, "CollapseTestSphereBubbleOuterTemperature[%"ISYM"][%"ISYM"] = %"GOUTSYM"\n", sphere, bubble, 
      BubbleOuterTemperature[sphere][bubble]);        
      /* 2018.04.24 */
