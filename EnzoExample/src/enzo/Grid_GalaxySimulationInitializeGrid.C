										 ...
										 float GalaxySimulationCR)
{
	...
	int DeNum, HINum, HIINum, HeINum, HeIINum, HeIIINum, HMNum, H2INum, H2IINum, DINum, DIINum, HDINum, B1Num, B2Num, B3Num, PhiNum;
	...
	if (ComovingCoordinates) {
		...
		CosmologyGetUnits(&DensityUnits, &LengthUnits, &TemperatureUnits, &TimeUnits, &VelocityUnits, Time);
	...
	/* set metals to small value. THIS MAY NEED TO BE MODIFIED. IF WE WANT "REAL" METAL FIELD.*/
	if (UseMetallicityField)
		for (i = 0; i < size; i++)
			BaryonField[MetalNum][i] = 1.0e-20; // modified 2018.02.21
	...
	float HII_Fraction, HeII_Fraction, HeIII_Fraction, HM_Fraction, H2I_Fraction, H2II_Fraction;

	HII_Fraction = 1.e-20;
	HeII_Fraction = 1.e-20;
	HeIII_Fraction = 1.e-20;
	HM_Fraction = 1.e-20;
	H2I_Fraction = 1.e-20;
	H2II_Fraction = 1.e-20;

	int tt0 = 0, ttem = 0, tte5 = 0, ene = 0;
			...
			for (i = 0; i < GridDimension[0]; i++, n++) {
				...
				if (r < DiskRadius) {
					...
					for (dim = 0; dim < 1+(HydroMethod == Zeus_Hydro ? GridRank : 0); dim++) {
					  ...
					  /* Compute z and r_perp (AngularMomentum is angular momentum and must have unit length). */    
						/* magnitude of z = r.L in L direction */
						zheight = AngularMomentum[0]*xpos + AngularMomentum[1]*ypos + AngularMomentum[2]*zpos;
						...
						FLOAT inv[3][3],temp;
						int rrr, sss;
						/* int i,j; */ // int i, j are already used....
						...
						/* 2018.01.31 */
						for (rrr=0;rrr<3;rrr++)
							for (sss=rrr+1;sss<3;sss++) {
								temp = inv[rrr][sss];
								inv[rrr][sss] = inv[sss][rrr];
								inv[sss][rrr] = temp;
							}						
						/* 2018.01.31 */
						...
						/* DiskDensity is determined by gas mass!! */
						/* This density is only calculated for the given ScaleHeightR!! */
						...
						if (PointSourceGravity > 0 )
							DiskVelocityMag = gasvel(drad, DiskDensity, ExpansionFactor, GalaxyMass, 
													ScaleHeightR, ScaleHeightz, DMConcentration, Time);
						else if( DiskGravity > 0 ) { 
							CellMass = gauss_mass(drad*LengthUnits,zheight*LengthUnits, xpos*LengthUnits, ypos*LengthUnits, 
								zpos*LengthUnits, inv, DiskDensity*DensityUnits,ScaleHeightR*Mpc, ScaleHeightz*Mpc, 
						...
						if (dim == 0 || dim == 1)
							Velocity[0] = DiskVelocityMag*(AngularMomentum[1]*xhat[2] - AngularMomentum[2]*xhat[1]);
						if (dim == 0 || dim == 2)
							Velocity[1] = DiskVelocityMag*(AngularMomentum[2]*xhat[0] - AngularMomentum[0]*xhat[2]);
						if (dim == 0 || dim == 3)
							Velocity[2] = DiskVelocityMag*(AngularMomentum[0]*xhat[1] - AngularMomentum[1]*xhat[0]);

					} // end: loop over dims

					/* If the density is larger than the background (or the previous disk), then set the velocity. */
					...
					// if (UseMetallicityField) // This should be converted to a general color field at some point - this obviously breaks metallicity feature
						// BaryonField[MetalNum][n] = density; // 2018.02.21 Comment out...
					}
				} // end: if (r < DiskRadius)
				...
				/* 2018.02.19 Follow the code in Grid_CollapseTestInitializeGrid.C */
				// Set multispecies fields!
				if (MultiSpecies > 0) {
					BaryonField[HIINum][n]   = HII_Fraction * CoolData.HydrogenFractionByMass * BaryonField[0][n];
					BaryonField[HeIINum][n]  = HeII_Fraction * BaryonField[0][n] * (1.0-CoolData.HydrogenFractionByMass);
					BaryonField[HeIIINum][n] = HeIII_Fraction * BaryonField[0][n] * (1.0-CoolData.HydrogenFractionByMass);
					BaryonField[HeINum][n]   = (1.0 - CoolData.HydrogenFractionByMass)*density -
												BaryonField[HeIINum][n] - BaryonField[HeIIINum][n];

					if (MultiSpecies > 1) {
						BaryonField[HMNum][n]  = HM_Fraction * BaryonField[HIINum][n];
						BaryonField[H2INum][n] = H2I_Fraction * BaryonField[0][n] * CoolData.HydrogenFractionByMass;
					...
					if (MultiSpecies > 1) // This "MultiSpecies" comes from "global_data.h".
