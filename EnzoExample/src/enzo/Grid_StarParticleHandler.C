...
extern "C" void FORTRAN_NAME(star_maker2)(int *nx, int *ny, int *nz,
  ...
  float *mp, float *tdp, float *tcp, float *metalf,
     int *imetalSNIa, float *metalSNIa, float *metalfSNIa, float *metalcrit);
...
extern "C" void FORTRAN_NAME(snfb_lihsin)(int *nx, int *ny, int *nz,
       float *d, float *dm, float *te, float *ge, float *u, float *v,
       float *w, float *metal,
       int *idual, int *imetal, hydro_method *imethod, float *dt,
       float *r, float *dx, FLOAT *t, float *z,
...
extern "C" void FORTRAN_NAME(pop3_simple_maker)
  (int *nx, int *ny, int *nz, 
   float *d, float *dm, float *h2d, float *temp, float *u, float *v, float *w, float *cooltime, 
   float *dt, float *r, float *metal, float *dx, FLOAT *t, float *z, int *procnum, 
   float *d1, float *x1, float *v1, float *t1, 
   int *nmax, FLOAT *xstart, FLOAT *ystart, FLOAT *zstart, int *ibuff, 
   int *imetal, hydro_method *imethod, float *h2crit, float *metalcrit, 
   float *odthresh, float *starmass, int *level, int *np, 
   FLOAT *xp, FLOAT *yp, FLOAT *zp, float *up, float *vp, float *wp, 
   float *mp, float *tdp, float *tcp, 
   float *metalf, float *justburn, float *masseff);

extern "C" void FORTRAN_NAME(pop3_simple_feedback)
 (
  int *npart, int *nx, int *ny, int *nz, 
  float *d, float *dm, float *te, float *ge, float *u, float *v, float *w, float *metal, 
  ...
  FLOAT *xp, FLOAT *yp, FLOAT *zp, 
  float *up, float *vp, float *wp,
  float *mp, float *life, float *tcp, 
  float *metalf, int *type, int *ctype, 
  int *distrad, int *diststep, int *distcells,
  int *sndistrad, int *sndistep, int *sndistcells,
  float *thermal_para);
      ...
      float dtLevelAbove, float TopGridTimeStep)
{
  ...
  int SIMPlE_POP3_TYPE = PARTICLE_TYPE_SIMPLE_POP3;
  ...
  if (STARMAKE_METHOD(POP3_STAR) || STARMAKE_METHOD(POP3_SIMPLE)) {
    //printf("sjlkdfjsdlf\n");
  ...
  if (STARMAKE_METHOD(NORMAL_STAR)) {
    ...
    FORTRAN_NAME(star_maker2)(
     ...
     &StarMakerTypeIaSNe, BaryonField[MetalIaNum], tg->ParticleAttribute[3], &PopIIIMetalCriticalFraction);
  ...
  if (STARMAKE_METHOD(POP3_SIMPLE)) {
    float PopIII_SIMPLE_MASS = PopIIIStarMass;
    ...
    if (PopIIIInitialMassFunction == TRUE) {
      ...
      dm = log10(PopIIIUpperMassCutoff / PopIIILowerMassCutoff) / (float) (IMF_TABLE_ENTRIES-1);
      m0 = log10(PopIIILowerMassCutoff);
      total_fn = 0;
      for (i = 0; i < IMF_TABLE_ENTRIES; i++) {
        m = POW(10.0, m0 + i*dm);
        total_fn += POW(m, PopIIIInitialMassFunctionSlope) * exp(-POW((PopIIIStarMass / m), CutoffExponent));
        IMFData[i] = total_fn;
      } // ENDFOR i
      ...
      float ddm = log10(PopIIIUpperMassCutoff / PopIIILowerMassCutoff) / (float) (IMF_TABLE_ENTRIES-1);
      ...
      while (width > 1) {
        width /= 2;
        if (xx > IMFData[bin_number])
          bin_number += width;
        else if (xx < IMFData[bin_number])
          bin_number -= width;
        else
          break;
      }
   
      PopIII_SIMPLE_MASS = PopIIILowerMassCutoff * POW(10.0, bin_number * ddm);
      //fprintf(stderr, "PopIII_SIMPLE_MASS from IMF: %fi\n", PopIII_SIMPLE_MASS);
    } 

    //---- POPULATION III (SINGLE STAR, simple)

    NumberOfNewParticlesSoFar = NumberOfNewParticles;

    FORTRAN_NAME(pop3_simple_maker)
    (GridDimension, GridDimension+1, GridDimension+2, 
     BaryonField[DensNum], dmfield, h2field, temperature, BaryonField[Vel1Num], 
     BaryonField[Vel2Num], BaryonField[Vel3Num], cooling_time, 
     &dtFixed, BaryonField[NumberOfBaryonFields], MetalPointer, 
     &CellWidthTemp, &Time, &zred, &MyProcessorNumber, 
     &DensityUnits, &LengthUnits, &VelocityUnits, &TimeUnits, 
     &MaximumNumberOfNewParticles, CellLeftEdge[0], CellLeftEdge[1], CellLeftEdge[2], &GhostZones, 
     &MetallicityField, &HydroMethod, &PopIIIH2CriticalFraction, &PopIIIMetalCriticalFraction, 
     &OverDensityThreshold, &PopIIIStarMass, &level, &NumberOfNewParticles, 
     ...
     tg->ParticleMass, tg->ParticleAttribute[1], tg->ParticleAttribute[0], tg->ParticleAttribute[2], 
     &RadiationData.IntegratedStarFormation, &PopIIIMakerMassEfficiency);

    for (i = NumberOfNewParticlesSoFar; i < NumberOfNewParticles; i++)
        tg->ParticleType[i] = SIMPlE_POP3_TYPE;
  ...
  ENZO_FAIL("Error in sink_maker.\n");
    ...
    if (ParticleType[n] == SinkParticleType && ParticleMass[n] == FLOAT_UNDEFINED)
  ...
  //---- THIS IS THE MODIFIED STAR FORMATION ALGORITHM
    // printf("MODIFIED STAR FORMATION ALGORITHM\n"); 
  ...
  if (STARFEED_METHOD(POP3_SIMPLE)) {
     //fprintf(stderr,"POP3_THERMAL_FEEDBACK %.3e\n", PopIIIEnergyToThermalFeedback);
     //fprintf(stderr, "nx:%d, ny:%d, nz:%d\n", GridDimension, GridDimension+1, GridDimension+2);    

     //---- POP III STAR FEEDBACK ALGORITHM
     FORTRAN_NAME(pop3_simple_feedback) (
     &NumberOfParticles, GridDimension, GridDimension+1, GridDimension+2, 
     BaryonField[DensNum], dmfield, BaryonField[TENum], BaryonField[GENum], 
     BaryonField[Vel1Num], BaryonField[Vel2Num], BaryonField[Vel3Num], MetalPointer, 
     &DualEnergyFormalism, &MetallicityField, &HydroMethod, 
     &dtFixed, BaryonField[NumberOfBaryonFields], &CellWidthTemp, &Time, &zred,
     &DensityUnits, &LengthUnits, &VelocityUnits, &TimeUnits,
     CellLeftEdge[0], CellLeftEdge[1], CellLeftEdge[2], &GhostZones,
     ParticlePosition[0], ParticlePosition[1], ParticlePosition[2], 
     ParticleVelocity[0], ParticleVelocity[1], ParticleVelocity[2], 
     ParticleMass, ParticleAttribute[1], ParticleAttribute[0], 
     ParticleAttribute[2], ParticleType, &SIMPlE_POP3_TYPE,
     &PopIIIStellarFeedbackDistRadius, &PopIIIStellarFeedbackDistCellStep, &PopIIIStellarFeedbackDistTotalCells, 
     &PopIIISimpleSupernovaDistRadius, &PopIIISimpleSupernovaDistCellStep, &PopIIISimpleSupernovaDistTotalCells,
     &PopIIIEnergyToThermalFeedback);
  } // end: if POP3_STAR



  if (STARFEED_METHOD(SINGLE_SUPERNOVA) ) { // John Forbes - Dec. 2013, this is typeII supernova
...
#define NO_PARTICLE_IN_GRID_CHECK   
  ...
  //----  Truly stochastic star formation and Supernovae.
    ...
    FORTRAN_NAME(star_feedback_ssn)(
      ...
      &MaximumRefinementLevel); /* This is for TypeII Supernovae. 2018.03.15*/
