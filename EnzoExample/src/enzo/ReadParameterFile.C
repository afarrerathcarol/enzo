...
int ReadParameterFile(FILE *fptr, TopGridData &MetaData, float *Initialdt)
{
  ...
  fprintf(stderr, "Warning: Incorrect version number.\n");
    ...
    ret += sscanf(line, "PopIIIInitialMassFunctionSub = %"ISYM, 
      &PopIIIInitialMassFunctionSub);   
    ...
    ret += sscanf(line, "PopIIIStellarFeedbackDistRadius = %"ISYM, 
      &PopIIIStellarFeedbackDistRadius);
    ret += sscanf(line, "PopIIIStellarFeedbackDistCellStep = %"ISYM, 
      &PopIIIStellarFeedbackDistCellStep);
    ret += sscanf(line, "PopIIISimpleSupernovaDistRadius = %"ISYM, 
      &PopIIISimpleSupernovaDistRadius);
    ret += sscanf(line, "PopIIISimpleSupernovaDistCellStep = %"ISYM, 
      &PopIIISimpleSupernovaDistCellStep);
    ret += sscanf(line, "PopIIIEnergyToThermalFeedback = %"FSYM, 
      &PopIIIEnergyToThermalFeedback);
    ret += sscanf(line, "PopIIIMakerMassEfficiency = %"FSYM, 
      &PopIIIMakerMassEfficiency);
 ...
 *  but only if it is not a fresh simulation*/
  ...
  */
  ...
  if ((STARFEED_METHOD(POP3_SIMPLE)) && (PopIIIStellarFeedbackDistRadius > 0)) {

    // Calculate number of cells in the shape over which to distribute feedback.
    PopIIIStellarFeedbackDistRadius = min(PopIIIStellarFeedbackDistRadius,
         PopIIIStellarFeedbackDistCellStep);
    int i, j, k, cell_step;

    PopIIIStellarFeedbackDistTotalCells = 0;
    for (k = -PopIIIStellarFeedbackDistRadius;k <= PopIIIStellarFeedbackDistRadius;k++) {
      for (j = -PopIIIStellarFeedbackDistRadius;j <= PopIIIStellarFeedbackDistRadius;j++) {
        for (i = -PopIIIStellarFeedbackDistRadius;i <= PopIIIStellarFeedbackDistRadius;i++) {
          cell_step = ABS(k) + ABS(j) + ABS(i);
          if (cell_step <= PopIIIStellarFeedbackDistCellStep) {
            PopIIIStellarFeedbackDistTotalCells++;
          }
        }
      }
    }
    if (MyProcessorNumber == ROOT_PROCESSOR) {
      fprintf(stderr,"Total cells for PopIII star feedback smoothing: %"ISYM".\n",
        PopIIIStellarFeedbackDistTotalCells);
    }
  }
  //fprintf(stderr, "ste distrad: %d\n", PopIIIStellarFeedbackDistRadius);
  //fprintf(stderr, "ste distcells: %d\n", PopIIIStellarFeedbackDistTotalCells);

  if ((STARFEED_METHOD(POP3_SIMPLE)) && (PopIIISimpleSupernovaDistRadius > 0)) {

    // Calculate number of cells in the shape over which to distribute feedback.
    PopIIISimpleSupernovaDistRadius = min(PopIIISimpleSupernovaDistRadius,
         PopIIISimpleSupernovaDistCellStep);
    int i, j, k, cell_step;

    PopIIISimpleSupernovaDistTotalCells = 0;
    for (k = -PopIIISimpleSupernovaDistRadius;k <= PopIIISimpleSupernovaDistRadius;k++) {
      for (j = -PopIIISimpleSupernovaDistRadius;j <= PopIIISimpleSupernovaDistRadius;j++) {
        for (i = -PopIIISimpleSupernovaDistRadius;i <= PopIIISimpleSupernovaDistRadius;i++) {
          cell_step = ABS(k) + ABS(j) + ABS(i);
          if (cell_step <= PopIIISimpleSupernovaDistCellStep) {
            PopIIISimpleSupernovaDistTotalCells++;
          }
        }
      }
    }
    if (MyProcessorNumber == ROOT_PROCESSOR) {
      fprintf(stderr,"Total cells for PopIII SNe feedback smoothing: %"ISYM".\n",
        PopIIISimpleSupernovaDistTotalCells);
    }
  }
  //fprintf(stderr, "SN distrad: %d\n", PopIIISimpleSupernovaDistRadius);
  //fprintf(stderr, "SN distcells: %d\n", PopIIISimpleSupernovaDistTotalCells);
