         ...
         LevelHierarchyEntry *LevelArray[], int level)
{
  ...
  for (i = 0; i < MAX_STATIC_REGIONS; i++)
    MaximumStaticSubgridLevel = max(MaximumStaticSubgridLevel, StaticRefineRegionLevel[i]);
  ...
  */
  ...
  if (MoveParticlesBetweenSiblings && level > max(MaximumStaticSubgridLevel,0))
  ...
  if (MetaData->StaticHierarchy == FALSE) {
    ...
    for (i = level; i < MAX_DEPTH_OF_HIERARCHY-1; i++) {
      ...
      */
      ...
      tt0 = ReturnWallTime();
      for (j = 0; j < subgrids; j++) {
        SubgridHierarchyPointer[j]->ParentGrid->GridData->DebugCheck("Rebuild parent");
        ...
        SubgridHierarchyPointer[j]->GridData->InterpolateFieldValues(SubgridHierarchyPointer[j]->ParentGrid->GridData
                                                                     ,TempLevelArray[i+1],MetaData);
      ...
      ChainingMeshStructure ChainingMesh;
      FastSiblingLocatorInitialize(&ChainingMesh, MetaData->TopGridRank,MetaData->TopGridDims);
      tt1 = ReturnWallTime();
      RHperf[9] += tt1-tt0;
    ...
    for (i = level; i < MAX_DEPTH_OF_HIERARCHY-1; i++) {
      ...
      for (j = 0; j < grids; j++)
        ...
        if (GridHierarchyPointer[j]->NextGridNextLevel != NULL) {
          ...
          GridHierarchyPointer[j]->GridData->ZeroSolutionUnderSubgrid(NULL, ZERO_UNDER_SUBGRID_FIELD);
          ...
          if (GridHierarchyPointer[j]->GridData->MoveSubgridStars(subgrids, ToGrids, FALSE) == FAIL)
