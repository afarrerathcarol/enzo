...
int Star::ComputePhotonRates(const float TimeUnits, int &nbins, float E[], double Q[])
{
  ...
  int i;
  double L_UV, cgs_convert, _mass, _Z;
...
//    	    this->last_accretion_rate, Q[0]); 
  ...
  case NormalStar:
    ...
    E[2] = 0;
    E[3] = 0; 
    // added 2018.12.21 LHC, to be calculated

    // nbins = 1;
    // E[0] = 21.0;  // Good for [Z/H] > -1.3  (Schaerer 2003)
    // Calculate Delta(M_SF) for Cen & Ostriker star particles
#ifdef TRANSFER
     _mass = max(min((float)(_mass), 100), 1); // only work for stars with mass between 1 and 30 Msun (Z=1.e-3)
    if (_mass >= 1 && _mass <= 100) {
      //switch((this->star_z)) for later use, may not be necessary
      if (this->Metallicity >= 1.e-2*0.02) {
        Q[0] = pow(10.0, 46.86 + 4.70*x  - 0.53*x2);
        Q[1] = pow(10.0, 40.64 + 7.72*x  - 0.94*x2);
        Q[2] = pow(10.0, 23.33 + 15.79*x - 2.00*x2);
        Q[3] = pow(10.0, 48.20 + 3.90*x  - 0.44*x2);
      } else if (this->Metallicity < 1.e-2*0.02 && this->Metallicity >= 1.e-3*0.02) {
        Q[0] = pow(10.0, 47.25 + 4.54*x  - 0.51*x2);
        Q[1] = pow(10.0, 41.39 + 7.40*x  - 0.90*x2);
        Q[2] = pow(10.0, 25.03 + 15.15*x - 1.93*x2);
        Q[3] = pow(10.0, 48.54 + 3.73*x  - 0.42*x2);
      } else if (this->Metallicity < 1.e-3*0.02) {
        Q[0] = pow(10.0, 47.63 + 4.36*x  - 0.49*x2);
        Q[1] = pow(10.0, 42.20 + 7.04*x  - 0.85*x2);
        Q[2] = pow(10.0, 27.36 + 13.97*x - 1.77*x2);
        Q[3] = pow(10.0, 48.83 + 3.58*x  - 0.40*x2);
      }
    } // ENDIF
    else {
      for (i = 0; i < nbins; i++) Q[i] = 0.0;
    }
    // added 2018.12.21 LHC, from MESA 1-D stellar evolution model, \barQ (averaged over lifetime)  
